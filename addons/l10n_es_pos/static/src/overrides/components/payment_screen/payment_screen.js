...
patch(PaymentScreen.prototype, {
    async validateOrder(isForceValidate) {
        if (this.pos.config.is_spanish) {
            ...
            if (!order.canBeSimplifiedInvoiced() && !order.to_invoice) {
            ...
            if (order.is_l10n_es_simplified_invoice) {
                // If the dedicated journal is set we will invoice for the simplified partner
                // else only closing session entries will be created.
